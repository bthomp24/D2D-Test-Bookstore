...
def parse_for_title(content):
    return Parent_Scrape.parse(content, ".//h1[@class='title_product']/div")[0].text

def parse_for_subtitle(content):
    subtitle_element = Parent_Scrape.parse(content, "//*[@class='value-field Subtitulo']")
    if len(subtitle_element)>0:
        subtitle = subtitle_element[0].text
        return subtitle

def parse_for_book_image_url(content):
    return Parent_Scrape.parse(content, "//*[@id='image-main']/@src")[0]

def parse_for_isbn(content):
    return Parent_Scrape.parse(content, "//*[@class='value-field ISBN']")[0].text

def parse_for_description(content):
    return Parent_Scrape.parse(content, "//meta[@itemprop='description']/@content")[0]

def parse_for_authors(content):
    authors_element = Parent_Scrape.parse(content, "//*[@class='value-field Colaborador']")[0]
...
def parse_for_book_id(content):
    return Parent_Scrape.parse(content, "//meta[@itemprop='gtin13']/@content")[0]

def parse_for_url(content):
    return Parent_Scrape.parse(content, "//meta[@itemprop='url']/@content")[0]
